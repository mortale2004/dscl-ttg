    ...
    return super.aggregate([
        {
            $lookup: {
                ...
                pipeline: [{$match:{$expr:{$eq:["$_id", "$$timeSlotId"]}}}],
                as: "timeSlotDetails",
            }
        },
        {
        ...
        },
        {
            $project:{
                ...
                dcya_id: "$_id.dcya_id",
                time_slots: 1,
                division_name: {$first:{$first:"$dcyaDetails.divisionDetails.division_name"}},
                course_name: {$first:{$first:"$dcyaDetails.courseDetails.course_name"}},
                course_sem_name: {$first:{$first:"$dcyaDetails.courseSemDetails.course_sem_name"}},
                classroom_name: {$first:{$first:"$dcyaDetails.classroomDetails.classroom_name"}},
            }
        }
    ], options, {
      filter,
