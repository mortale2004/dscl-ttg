...
const dayNames = Object.entries(SYSTEM_CONSTANT.DAY).reduce((acc, [key, value]) => {
  acc[key] = value;
  return acc;
}, {});
...
const TimeTableDisplay = ({ data, printTableRef }) => {
  ...
  const findSlot = (dayNumber, timeSlotId) => {
    return timeSlotsByDay[dayNumber]?.find(slot => slot.time_slot_id === timeSlotId);
  };
    ...
    if (slot.is_break_slot) {
      return (
        <Box sx={{ textAlign: "center", p: 1, bgcolor: "grey.100" }}>
          <Typography variant="body2" fontWeight="bold">{slot.break_name}</Typography>
    ...
    return (
      <Box sx={{ p: 1 }}>
        ...
        <Typography variant="caption">
          {slot.teacher_last_name} {slot.teacher_first_name} {slot.teacher_father_name}
        </Typography>
      </Box>
    );
  };
  ...
  return (
    <Box sx={{ p: 2 } } ref={printTableRef}>
      <Box sx={{ mb: 3, textAlign: "center" }}>
        <Typography variant="h5" gutterBottom>
          {course_short_name} {course_sem_name} - Division {division_name}
        </Typography>
        <Typography variant="subtitle1">
          {course_name} | Academic Year: {academic_year_name} | Classroom: {classroom_name}
        </Typography>
      </Box>
        ...
        <Table sx={{ minWidth: 650 }}>
          <TableHead>
            <TableRow sx={{ bgcolor: "primary.main" }}>
              <TableCell sx={{ color: "white", fontWeight: "bold" }}>Day / Time</TableCell>
              {allTimeSlots.map(timeSlot => (
                <TableCell key={timeSlot.id} sx={{ color: "white", fontWeight: "bold" }}>
                  {timeSlot.name}
                </TableCell>
              ))}
            </TableRow>
          ...
          <TableBody>
            {Object.entries(dayNames).map(([dayNumber, dayName]) => (
              <TableRow key={dayNumber} sx={{ '&:nth-of-type(odd)': { bgcolor: 'action.hover' } }}>
